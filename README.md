...
- `service_codon_balance_comparison.csv` — for every NCBI
  translation table and both sense-pool models,
...
This dataset asks what the four service codons would have to carry for
the two halves of the third-position Keto/Amino partition to balance,
and how that answer changes when global divisibility by 37 is demanded
in addition. It covers all 27 NCBI translation tables under both
sense-pool models, one row per table and model, 54 rows in
total.

#### The two procedures

For each quantity `Q` in {P, N} independently, write `Q_pool` for the
sense-pool total of the table under the model in question,
...
Proton and neutron components are solved independently; neither
equality of the two stop parameters nor any offset on ATG is imposed,
so where those appear they are results rather than assumptions.
...
denote absence. Where a table has no stop position at all, the stop
parameter does not exist and its column stays empty while the ATG
column is filled. Boolean columns are left empty rather than set to
...
- `Mod_Stop_Params_Equal` — whether the two stop parameters coincide
  under the divisibility procedure; computed only where stop positions
