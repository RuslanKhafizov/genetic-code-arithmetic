...
4. The core analysis is reproduced by one self-contained
   ...
   input datasets and generates seventeen computed
...
5. The seventeen **computed datasets** themselves,
...
All seventeen computed datasets are generated by `reproduce.py` from
...
- `reproduce.py` — the Python script that generates all
  seventeen computed datasets from the five input datasets. See the
...
for, any of the seventeen computed datasets above. See the dedicated section
...
All seventeen computed datasets in this repository are generated from
...
directory and writes the seventeen computed CSV files to the same
...
input files produces seventeen output files that are identical
...
together with the other sixteen computed datasets in this
