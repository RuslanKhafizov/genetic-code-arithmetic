...
adds no new structure: it is the unique minimal non-negative integer
...
The systems in Sections~\ref{sec:results-key1-neutrons}--\ref{sec:results-key1-protons}
use quantities established without assigning values to the
service codons (Section~\ref{sec:results-independent}): the
differences $37$ for neutrons and $36$ for protons, localized
in the tryptophan--isoleucine pair
(Section~\ref{sec:results-key1-trpile}), and the pool sums
$N(\mathcal{C}_{\mathrm{SE}})=97\cdot37$ and
$P(\mathcal{C}_{\mathrm{SE}})=113\cdot37-1$
(Section~\ref{sec:results-sensepool}). Parity rules out odd
proton values but does not determine the size of the deviation
(Section~\ref{sec:results-proton-parity}).
The minimal neutron contribution required for balance is
$111=3\cdot37$; after it is added, the total sum coincides
with $T(\mathrm{Octet~I})$. This Octet~I sum is not a target
used to select the parameters. On the proton side, divisibility
increases the minimal contribution by four: one proton for
each service position. Coprimality of $4$ and $37$ is used
to solve the congruences; the resulting equality
$p_{\text{stop}}=n_{\text{stop}}=37$ is not prescribed.
The comparison of initial quantities and assignments across
the NCBI tables is given in Section~\ref{sec:results-key1-ncbi}.
