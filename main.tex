...
% Portable filenames: line breaks are allowed after underscores.
...
\begin{abstract}
We analyze proton and neutron counts of free neutral amino acids
across $33$ codon groups of the standard genetic code, defined
by two natural grouping systems: the Rumer partition and the
three third-position chemical axes---Keto/Amino, Strong/Weak,
and Purine/Pyrimidine. After excluding the three stop codons and the initiator
$\mathrm{ATG}$, the neutron sum of the remaining $60$ codons
is divisible by~$37$, while the proton sum is one below
the nearest multiple. The same pattern appears in the
Keto/Amino asymmetry, which reduces to the tryptophan--isoleucine
pair with neutron and proton differences of $37$ and $36$.
Proton parity excludes a difference of $37$
but does not determine the one-unit deviation. To analyse the symmetries of the complete table, we derive
equal formal contributions for the three stop codons
and a separate contribution for the initiator. Keto/Amino
balance and divisibility of the global proton and neutron
sums by~$37$ yield a unique minimal non-negative integer
solution. The three stop codons contribute $111=3\cdot37$ neutrons
and the initiator none, equating the global neutron sum
with the Octet~I nucleon sum without imposing this equality. Across $27$ NCBI tables, the neutron deficit relative
to the Octet~I nucleon sum and the proton residue
recur only with the same service-excluded pool.
Controls highlight third-position groupings and~$37$
under chosen measures. Monte Carlo comparison with random permutations of amino acids
among synonymous blocks shows an atypical proximity of the
standard-code group sums to multiples of~$37$; the result persists
when methionine is held fixed. These group-sum patterns depend
jointly on codon multiplicity and nucleon composition.
\end{abstract}
...
In the present work, the arithmetic regularities of
the standard genetic code are examined within this
single grouping system. We identify the initial
numerical relations and trace the equalities and
divisibilities that follow from them across different
parts of the codon table.

To assess how specific these regularities are
to the standard code, we compare it with alternative
NCBI codes and use Monte Carlo simulations to estimate
how often comparable results arise under random
permutations of amino acids among the synonymous
blocks. We also examine whether the third codon
position stands out among the three positions
and the number~$37$ among the other prime moduli,
and how the results depend on the molecular form
...
nucleon composition of a physical ensemble of molecules. The notation $P$, $N$, $T$, and $\Delta$ is retained
for consistency with the underlying amino acid contributions,
but no biochemical interpretation is claimed for the formal
Key~1 contributions.
...
writes 18 computed output datasets. A separate deterministic script,
\texttt{controls.py}, writes four control datasets. The main
statistical calculation is performed by \texttt{mc\_significance.py},
and the additional checks by \texttt{mc\_sensitivity.py}. They write
\texttt{mc\_significance.csv} and \texttt{mc\_sensitivity.csv},
respectively. The results are independently verified by
\texttt{verify\_mc.py} and \texttt{verify\_mc\_sensitivity.py}.
By default, all data paths are resolved relative to the directory
...
The remaining three outputs of the core script compare the standard
...
\texttt{service\_codon\_balance\_comparison.csv} contains
$54$ rows: one for each table and each model.
For protons and neutrons separately, non-negative integer
assignments are determined by minimizing the total
contribution of the service codons. All stop codons
in the chosen model receive the same value,
while $\mathrm{ATG}$ receives a separate value.
Two procedures are compared: Keto/Amino balance alone
and the same balance with the additional requirement
that the complete-table sum be divisible by~$37$.
The file records the assignments, added contributions,
resulting sums, and the difference between the minimal
contributions under the two procedures. It distinguishes
a unique assignment, a determined total contribution
without a unique assignment, and the absence of a solution.
The application of these procedures to the standard code
is presented in Section~\ref{sec:results-key1-derivation}.
...
randomization. Randomization is used in the main Monte Carlo
calculation (\texttt{mc\_significance.py}), the additional checks
(\texttt{mc\_sensitivity.py}), and their independent
implementations (\texttt{verify\_mc.py} and
\texttt{verify\_mc\_sensitivity.py}).
Random seeds and numbers of trials are specified in all cases.
Tail-probability estimates and intervals use floating-point values;
tail membership is determined without rounding the statistics.
...
the source of published numerical results: the deterministic
numerical values reported in the main text are checked against the
corresponding CSV files. An online
...
\end{itemize}
These four files contain the results of three controls---codon
position, choice of modulus, and molecular representation---and
of the cross-table comparison of Keto/Amino differences.
The interpretation is given in
Section~\ref{sec:discussion-controls}.

\paragraph{Data and code availability.}
The accompanying repository contains the manuscript sources, five
input datasets, 18 core computed datasets, four control datasets, the
two Monte Carlo summary tables \texttt{mc\_significance.csv} and
\texttt{mc\_sensitivity.csv}, six Python scripts and the interactive
visualizer. Complete run commands and
...
figures were generated by the accompanying \texttt{visualization.html}.}
\label{fig:pool60}
\end{figure}
...
\begin{figure}[!htbp]
  ...
  complete $64$-codon table. Generated by the accompanying
...
For each group $G\in\{\mathrm{Amino},\mathrm{Weak}\}$,
the ratios to the nucleon sum under Key~0 form a profile
with a common denominator of~$13$:
\[
...
Under Key~1, the Keto, Amino, Strong, and Weak groups
have identical values of the four nucleon quantities.
For each of these groups $G$:
\[
\begin{aligned}
...
\end{aligned}
\]
Keto/Amino balance, together with the twin-group identities,
ensures equality of the sums of all four groups
in $P$ and $N$, while the definitions
...
the same ratios of nucleon quantities under Key~1.
Denoting any of these groups by $G$, we obtain:
\[
\frac{P_1(G)}{T_1(G)}=\frac{29}{54},\qquad
\frac{N_1(G)}{T_1(G)}=\frac{25}{54},\qquad
\frac{\Delta_1(G)}{T_1(G)}=\frac{2}{27}.
\]
Accordingly, $P_1(G):N_1(G)=29:25$.
...
$37$. For each of the four aligned groups $G$, we have
\[
\begin{aligned}
...
The nucleon sum of each of the four aligned halves
under Key~1 is $3996=4\cdot999$; this follows by halving
the total sum.
...
$32$-codon halves. After subtracting the equal Octet~I contributions,
each of the groups
$\{\mathrm{G}\}\cap\mathrm{Octet~II}$ and
$\{\mathrm{A}\}\cap\mathrm{Octet~II}$
under Key~1 has a nucleon sum of $1023$,
a proton sum of $548$, and a neutron sum of $475$.
...
$1096$ and $1196$, respectively. They deviate by $\pm50$ from the mean values:
$1000$ for the neutron sum and $1146$ for the proton sum.
These are the sums attained under Key~1 by the halves
of this octet along the Keto/Amino and Strong/Weak axes. This is another expression of the
...
methionine's total nucleon count, $149=4\cdot37+1$,
...
Section~\ref{sec:discussion-trpile-robustness} compares
the standard code with alternative NCBI codes in terms
of the neutron deficit and proton residue of the
service-excluded pool, the localization of the asymmetry
in the tryptophan--isoleucine pair, and the results of
re-deriving the service-codon assignments;
...
The comparison of the $27$ translation tables in the NCBI registry
shows which arithmetic properties of the standard code occur in
other genetic codes. For the service-excluded pool we compare the
neutron deficit and the proton residue
(Section~\ref{sec:results-sensepool}), together with the
differences of the neutron and proton sums between Keto and Amino.
For the tables that reproduce the standard values of these
differences, we check whether their localization in the
tryptophan--isoleucine pair is preserved
(Section~\ref{sec:results-key1-trpile}). Separately, the
same conditions of balance, divisibility and minimality are
applied to each table (Section~\ref{sec:results-key1-derivation}),
and the resulting assignments to the
service codons and the group sums are compared.

The same two models are used in all comparisons. Model~1 excludes
$\mathrm{ATG}$ and every codon listed in the registry as a stop
codon;
Model~2 excludes $\mathrm{ATG}$ and only the permanent stop
codons.
Context-dependent stop codons are included in Model~2 with the
amino acid assignments recorded for them. The distinction affects
tables $27$, $28$ and $31$. Excluding $\mathrm{ATG}$ alone while
retaining the
elongation assignments of the alternative start codons is a
convention shared by the two models
(Section~\ref{sec:genetic-code}).

\paragraph{Neutron deficit and proton residue.}
For the standard code the neutron sum of the service-excluded pool
falls short of the nucleon sum of Octet~I by
$3700-3589=111=3\cdot37$, and the proton
sum has residue $-1$ modulo~$37$. The first test checks whether
these two values are reproduced in the other tables.

In \texttt{deficit\_models\_analysis.csv} the neutron deficit is
computed relative to three reference
sums: the constant $3700$ of the standard code; the nucleon sum of
structural Octet~I, that is, of the same eight codon families as
in the standard code but under the amino acid assignments of the
table tested; and the nucleon sum of functional Octet~I, that is,
of all fully four-fold degenerate families of the table itself
(Section~\ref{sec:datasets}). All three deficits are determined
for each of the two pool
models, which gives six variants of the comparison. The proton
residue is taken modulo~$37$ directly from the proton sum of the
service-excluded pool, separately for each model, and requires no
reference sum.

The neutron deficit $111$ under any of the three reference sums
and
the proton residue $-1$ modulo~$37$ each select the same tables:
$1$, $11$ and $28$ under Model~1 and only $1$ and $11$ under
Model~2.
In the remaining $24$ tables neither of the two features is
reproduced under either model.

Table~11 has the same amino acid assignments and stop codons as
the standard table. In table~28 the exclusion of three
context-dependent stop codons under Model~1 leaves the same pool;
their return under Model~2 breaks both matches. The matching rows
therefore represent one and the same arithmetic of the original
pool rather than independent repetitions of the result.

\paragraph{Preservation of the Keto/Amino asymmetry and its localization.}
The second test checks whether the differences of the neutron and
proton sums, Keto minus Amino, are reproduced; in the standard
code they equal $37$ and $36$ respectively. The values for all
tables and both models are given in
\texttt{keto\_amino\_balance\_models.csv}. This pair of
differences
is reproduced in tables $1$, $6$, $11$, $29$ and $30$ under both
models, and in tables $27$ and $28$ only under Model~1. Agreement
of
the differences does not by itself establish their localization;
its preservation is checked against the codon assignments.

In tables $6$, $29$ and $30$ the codons $\mathrm{TAA}$ and
$\mathrm{TAG}$ are reassigned
to one and the same amino acid: glutamine, tyrosine or glutamic
acid respectively. Within the $\mathrm{TA}$ family the codon
$\mathrm{TAG}$ belongs to
the Keto branch and $\mathrm{TAA}$ to the Amino branch, so the
added
contributions cancel in the difference. There are no other changes
in these tables: $\mathrm{TGG}$ and all three isoleucine codons
retain their
standard assignments, and the localization in the
tryptophan--isoleucine pair is preserved. The same mechanism
operates in table~27 under Model~1: $\mathrm{TAA}$ and
$\mathrm{TAG}$ encode glutamine,
and the context-dependent $\mathrm{TGA}$ is excluded. Under
Model~2 its
tryptophan contribution breaks the match. Tables $11$ and $28$
under Model~1 retain the original pool of the standard code;
table~11 retains it under Model~2 as well.

Under Model~1 the pair of differences is reproduced by the
standard table, five alternative nuclear tables and table~11,
which is used by bacteria, archaea and plastids. None of the $13$
mitochondrial tables reproduces it. The failure is not due to the
reassignment of $\mathrm{ATA}$ or $\mathrm{TGA}$ alone: in tables
$16$, $22$ and $23$ both
codons retain their standard assignments, whereas in the remaining
ten at least one of them is changed. In table~16 $\mathrm{TAG}$
encodes
leucine; in table~22 the conversion of $\mathrm{TCA}$ into a stop
codon is
added to this; in table~23 $\mathrm{TTA}$ becomes a stop codon.
After these
reassignments additional contributions enter the difference of the
Keto and Amino sums, so that it no longer reduces to the
difference of the nucleon counts of tryptophan and isoleucine.

\paragraph{Preservation of the differences and divisibility of the sums.}
In the standard service-excluded pool the neutron sums of Keto and
Amino are divisible by~$37$. The proton sum of Amino is also
divisible by~$37$, while the proton sum of Keto has residue $-1$
modulo~$37$. This set of residues is reproduced by exactly the
same
tables as the standard values of the neutron deficit and the
proton residue in the first test: $1$, $11$ and $28$ under Model~1
and $1$, $11$ under Model~2.

In tables $6$, $27$, $29$ and $30$ the symmetric reassignment of
$\mathrm{TAA}$
and $\mathrm{TAG}$ contributes equally to both halves, separately
for protons
and for neutrons; these contributions are not divisible by~$37$.
In
the models where the standard differences are preserved, none of
the four sums is divisible by~$37$: the neutron residues are equal
and non-zero, and the proton residue of Keto is one less than the
proton residue of Amino.

The localization of the asymmetry in the tryptophan--isoleucine
pair is thus preserved under the symmetric reassignment of
$\mathrm{TAA}$ and
$\mathrm{TAG}$ as well. Equal increments to both halves, however,
preserve
their differences while changing the residues of the sums
themselves modulo~$37$. Preservation of this local asymmetry
therefore does not amount to reproducing the arithmetic
regularities of the original pool of the standard code.

\paragraph{Comparison of the minimal assignments.}
For each table and both models the dataset
\texttt{service\_codon\_balance\_comparison.csv} lists two
minimal assignments: under Keto/Amino balance alone and with the
global divisibility by $37$ added. In each procedure every stop
codon of the adopted model receives the same value and
$\mathrm{ATG}$ a
separate one; the total added contribution is minimized under
non-negative integer parameters, separately for $P$ and $N$. The
parameters are derived anew for each table.

Balance with divisibility has a joint solution in protons and
neutrons not for every table: under Model~1 it is absent for
tables $14$, $25$ and $33$, and under Model~2 also for $27$, $28$
and
$31$. Under balance alone a joint solution is absent only for
table~25. For tables $14$ and $33$ the minimal total contribution
is
determined separately for protons and for neutrons, but its
distribution among the service codons is not uniquely determined.

Under balance alone the minimal neutron compensation leads to
equality of the neutron sum of the complete table with the nucleon
sum of its structural Octet~I only in tables $1$, $11$ and $28$
under Model~1 and $1$, $11$ under Model~2. Neither this equality
nor divisibility by $37$ is imposed in the procedure. For the
standard code the result is $37$ neutrons for each stop codon and
a
zero contribution from $\mathrm{ATG}$; the sum equals
$3589+3\cdot37=3700$.

With divisibility additionally required the result broadens:
equality with the nucleon sum of structural Octet~I also holds for
tables $2$, $9$, $10$, $13$, $21$ and $24$ under both models.
Among
the tables with a joint solution of balance with divisibility this
equality is attained in $9$ of $24$ tables under Model~1 and in
$8$
of $21$ under Model~2. After the minimal assignment in both
components, these six tables agree with the standard code under
Key~1 in all four quantities across all $33$ groups. The
assignments themselves, however, differ. For table~2, for
instance, the result is $(P,N)=(26,24)$ per stop codon and
$(80,69)$ for $\mathrm{ATG}$, and
for table~10 $(10,17)$ and $(28,20)$ respectively. Table~10
differs from the
standard table only in reading $\mathrm{TGA}$ as cysteine; in the
other
five, further amino acid assignments are changed as well.

For tables $12$ and $26$ the proton and neutron sums after this
procedure also coincide with the standard values $4292$ and
$3700$, but
the full group profile differs. Agreement of the complete sums,
including the nucleon sum $7992=8\cdot999$, therefore does not
amount to
agreement of the sums of the individual groups.

The same minimal assignment as for the standard code --- $(37,37)$
for
each stop codon and $(1,0)$ for the initiator $\mathrm{ATG}$ ---
is obtained only
for tables $1$, $11$ and $28$ under Model~1 and $1$, $11$ under
Model~2. What is distinguished is the form of the assignment
rather than the magnitude of the added contribution itself: tables
$9$, $10$, $13$, $21$ and $24$ reproduce the full profile with a
smaller total added contribution in both $P$ and $N$.

The additional contribution required to satisfy divisibility once
the minimal balance has been reached amounts, for the standard
code, to four protons and zero neutrons. Among the tables with a
joint solution this is the smallest increment in each component
separately. The same values are obtained for table~11 under both
models and for table~28 under Model~1. For the remaining tables
with a joint solution both increments are larger.

The standard values of the neutron deficit and the proton residue
are thus reproduced only in tables with the same service-excluded
pool. The localization of the asymmetry in the
tryptophan--isoleucine pair is preserved under some changes of
that pool as well. After the assignment to the service codons the
agreements broaden: other codes can reproduce the entire group
profile of the standard code, but at different parameter values.
Agreement of the resulting sums therefore does not by itself
amount to agreement of the underlying arithmetic or of the
assignment to the service codons.

These conclusions concern the tables of the registry examined
here. The number of matches does not provide an estimate of
probability, since the tables are not an independent random sample
of genetic codes.
...
The third codon position, the modulus~$37$ and the representation of
free neutral amino acids define the framework of the analysis
(Sections~\ref{sec:partitioning}, \ref{sec:criteria}
and~\ref{sec:composition}). What follows tests which properties are
preserved when these choices are changed. The positional control
compares the divisibility of the sums along the same chemical axes;
the prime scan, how widely divisibility occurs across different
moduli; the representation control, the stability of the absolute
sums and of the differences. An additional a posteriori observation
on the Keto/Amino asymmetry across the three positions is then
considered.

\paragraph{Codon position.}
We take the positional control over the service-excluded pool: its
sums do not depend on the assignments to the service codons. The
three chemical axes are applied to each position in turn. At the
third position the Amino and Weak groups have all four quantities
divisible by~$37$; the Keto and Strong groups have their neutron
sums divisible by~$37$, and the Pyrimidine group its
difference~$\Delta$ (Section~\ref{sec:results-sensepool}). For Keto
and Strong the proton and nucleon sums lie one unit below the
nearest multiple of~$37$:
\[
\begin{aligned}
...
\end{aligned}
\]
Since the neutron sums are divisible by~$37$, the identities $T=P+N$
and $\Delta=P-N$ carry the residue of the proton sum over to $T$ and
$\Delta$. At the first position only the neutron sums of Keto and
Amino are divisible. Given the divisibility of the total sum of the
pool, $N(\mathcal C_{\mathrm{SE}})=3589=97\cdot37$, divisibility of
one of these halves forces divisibility of the other, so the two
matches are not independent. At the second position none of the four
quantities of any of the six groups is divisible by~$37$.

The chemical axes split the full table into halves of $32$ codons,
but once the service positions are excluded the sizes may differ. In
particular, Keto and Amino at the first position contain $29$ and
$31$ codons, and at the third position $30$ each. In the pool
considered here the divisibilities of the group sums are thus
concentrated at the third position; carrying the same chemical
classifications over to the other positions does not reproduce this
set of divisibilities. The values for all three positions in the
service-excluded pool (identified in the file by the value
\texttt{sense}), under Key~0 and under Key~1 are given in
\texttt{position\_axis\_analysis.csv}.

\paragraph{Choice of modulus.}
The scan covers all primes up to~$97$ and three sets of group
quantities: the values of $T$, $P$, $N$ and $\Delta$ for those $17$
groups of the $33$ that contain no service codons; the values of all
$33$ groups under Key~0; and the values of the same groups under
Key~1. In the file the sets are identified by the values
\texttt{param\_independent}, \texttt{key0} and \texttt{key1}. In the
first set the assignments to the service codons do not enter the
sums; in the second, divisibility by~$37$ is not imposed; and in the
main derivation of Key~1 it enters the conditions on the full sums
$P$ and $N$.

For each prime $p$ we count the share of the distinct non-zero
values that it divides. Repeated numerical values are counted once.
This share is compared with the value $1/p$, which corresponds to a
uniform distribution of residues. In all three sets the largest
ratio to this value belongs to~$37$: it divides $5$ of the $28$
distinct values in the first set, $13$ of $70$ under Key~0 and $10$
of $47$ under Key~1. The corresponding ratios are approximately
$6.61$, $6.87$ and $7.87$. For $2$ and $5$ they are below three in
all three sets; divisibility by these primes is connected,
respectively, with the parity of the proton counts and with decimal
roundness, discussed in Sections~\ref{sec:results-proton-parity}
and~\ref{sec:results-synthesis}.

The ratio to $1/p$ is a descriptive statistic: the scan specifies no
null model of genetic codes and yields no $p$-value. If $k$ of the $m$ distinct
values are divisible by $p$, the ratio equals $pk/m$: it does
not exceed $p$ and changes in steps of $p/m$. The range and
the step of the statistic therefore depend on the size of the
modulus. Removing duplicate values does not remove the dependencies induced by the
addition and the scaling of the group sums. The range of comparison
is limited to the primes up to~$97$. Powers of primes impose
additional divisibility conditions and are not part of this
comparison. It is established separately that no value in the three
sets is divisible by~$37^2$.

In addition, each positive value is reduced to its odd core by
removing all factors of~$2$. This eliminates repetitions of the form
$v$, $2v$ and $4v$ that arise, in particular, from the exact scaling
of the Octet~I groups (Section~\ref{sec:results-octetI}). Under
Key~0 the number~$37$ divides ten distinct odd cores, whereas the
multiples of each of $73$ and $61$ reduce to a single core. For $73$
this is the value itself, $73=2\cdot37-1$, from the Octet~II
$\Delta$ cascade; for $61$ it is the value $549=9\cdot61$, to which
the proton sums of the pyrimidine groups reduce: $2196=4\cdot549$
and $1098=2\cdot549$. This count does not establish the independence
of the remaining values. The full counts are given in
\texttt{prime\_divisibility\_scan.csv}. The first place of~$37$ under the
chosen metric is already observed in the set for the groups without
service codons and under Key~0; it is not due to the Key~1
assignments.

\paragraph{Dependence on the molecular representation.}
We compare the adopted representation of the free neutral amino acid
with the peptide residue and with the zwitterion, keeping the
isotope convention of Section~\ref{sec:composition}. In the
peptide-residue model one water molecule, that is $10$ protons and
$8$ neutrons, is subtracted from the formula of each amino acid. The
zwitterionic form has the same atomic composition as the free
neutral molecule, so its nucleon counts coincide with the original
ones.

These changes are described by a homogeneous integer shift
$(s_P,s_N)$: the same pair of values is added to the counts of every
amino acid. In the difference of two sums of one quantity the
additions cancel if the number of summands is the same. The
differences of the molecular counts of tryptophan and isoleucine are
therefore preserved, $\delta N(\mathrm{Trp},\mathrm{Ile})=37$ and
$\delta P(\mathrm{Trp},\mathrm{Ile})=36$. The differences of the
Keto/Amino and Strong/Weak sums at the third position are preserved
as well: in the service-excluded pool each of these groups contains
$30$ codons. This is stability with respect to homogeneous shifts;
an arbitrary change of the molecular representation may change
different amino acids differently.

In the absolute sums equal additions do not cancel. Divisibility of
the neutron sum of the pool by~$37$ is preserved under the condition
$60s_N\equiv0\pmod{37}$, that is $s_N\equiv0\pmod{37}$, since $60$
and $37$ are coprime. For the peptide residues the neutron and
proton sums are, respectively,
\[
\begin{aligned}
...
\end{aligned}
\]
Both lose their original residues modulo~$37$. The neutron sums of
Keto and Amino at the third position become $1573$ and $1536$: the
difference~$37$ is preserved, but neither sum is a multiple of~$37$
any longer. The nucleon sum of Octet~I in the same representation
equals $3700-32\cdot18=3124$, so its difference from the neutron sum
of the pool is $3124-3109=15$ instead of the original~$111$.

The absolute arithmetic properties thus belong to the adopted
molecular representation. The differences between groups of the pool
of equal cardinality and between molecules are preserved under the
homogeneous shifts tested here, but the stability of the whole
structure does not follow from this. All $13$ quantities tested
under the three representations are given in
\texttt{representation\_sensitivity.csv}.

\paragraph{The Keto/Amino asymmetry across the three positions.}
We now compare the differences of the Keto minus Amino sums in the
service-excluded pool. In the standard code, at each position
exactly one of the four differences in $T$, $P$, $N$ and $\Delta$ is
divisible by~$37$:
\[
\begin{array}{c@{\qquad}ccc}
\text{Position} & 1 & 2 & 3 \\
\text{Quantity} & N & T & N \\
\text{Difference} & -333=-9\cdot37 & 185=5\cdot37 & 37
\end{array}
\]
At the second position the divisibility arises in the subtraction:
the nucleon sums themselves are not multiples of~$37$, but they have
equal residues, which cancel in the difference.

At the first two positions the contributions of all nineteen amino
acids of the pool remain in the differences (methionine drops out
together with $\mathrm{ATG}$). At the third position synonymy
cancels the contributions of every amino acid except tryptophan and
isoleucine. The neutron difference of the two groups thereby reduces
to the difference of a single pair of molecules
(Section~\ref{sec:results-key1-trpile}):
\[
...
The comparison of the $27$ NCBI tables shows that the combination of
divisibilities in $N$, $T$, $N$ across the three positions is
reproduced only in tables $1$, $11$ and $28$ under Model~1 and in
tables $1$ and $11$ under Model~2. In all these cases the
composition of the analyzed pool and its amino acid assignments
coincide with the standard ones. For table~$28$ this agreement is
obtained when the context-dependent stop codons are excluded under
Model~1; Model~2 returns them to the pool
(Section~\ref{sec:discussion-trpile-robustness}). In the remaining
tables the divisibility for this sequence is preserved at no more
than one position. The full results are given in
\texttt{position\_asymmetry\_ncbi.csv}.

The sequence $N$, $T$, $N$ was singled out after inspection of the
data. The comparison of the tables describes how widely this
combination occurs in the registry and gives no estimate of the
probability of its arising by chance.

All four control datasets are computed deterministically in exact
integer arithmetic by the script \texttt{controls.py}
(Section~\ref{sec:datasets}).

\subsection{Significance under a random-code null}
\label{sec:discussion-montecarlo}

The comparison with natural codes and the deterministic controls
(Sections~\ref{sec:discussion-trpile-robustness}
and~\ref{sec:discussion-controls}) describe the specificity of
the observed structure. Comparing natural codes does not by itself
determine the probability of the observed properties under random
permutations of amino acids. Here we estimate how often randomly
reassigning amino acids to the blocks of the standard code
produces a comparable concentration of group quantities at
and near multiples of $37$. Comparisons with random alternatives
are used in studies of genetic-code optimality~\citep{freeland1998}.

The null model preserves the $64$ codons, the twenty synonymous
blocks with their sizes, the three stop codons, the Rumer
partition, and the third-position chemical axes. In each trial,
the twenty real amino acids are uniformly randomly permuted
among these blocks; each amino acid's proton and neutron counts
are transferred together. Six main summaries are considered.
\texttt{S1}--\texttt{S3} and \texttt{S4sense} are evaluated in
the service-excluded pool ($60$ codons): $\mathrm{ATG}$,
$\mathrm{TAA}$, $\mathrm{TAG}$, and $\mathrm{TGA}$ are excluded
from every group in every trial. \texttt{S4a} and \texttt{S4b}
extend the analysis to the complete table under Key~0 and Key~1,
respectively.

For each of the six main summaries, $M=10^{6}$ trials give the estimated tail
probability $p_{\mathrm{MC}}=(b+1)/(M+1)$, where $b$ is the
number of random codes with a result at least as extreme as
the observed one. Equal or greater values are counted for
divisibility counts, and equal or smaller values for mean
distances. Each estimate is accompanied by a $95\%$ Wilson
interval for the proportion $b/M$, describing Monte Carlo uncertainty.
The calculation is implemented in \texttt{mc\_significance.py}
with the random seed \texttt{seed=0};
the summary identifiers and results are reported in
\texttt{mc\_significance.csv}. The independent implementation
\texttt{verify\_mc.py} uses a different construction of the
codon groups, a direct search when deriving Key~1, and a different
random seed (\texttt{seed=12345}). It reproduces the observed
statistics exactly and the tail probabilities within the expected
Monte Carlo error.

\texttt{S1} tests joint divisibility by $37$ of the neutron
sum of the service-excluded pool and the nucleon sum of
Octet~I. In the standard code these are
$N(\mathcal{C}_{\mathrm{SE}}) = 3589 = 97 \cdot 37$ and
$T(\mathrm{Octet~I}) = 3700 = 100 \cdot 37$.
In random codes, both sums are tested for divisibility without
requiring them to match their exact values in the standard code.
The result is $p_{\mathrm{MC}} = 0.000800$ with a $95\%$ interval
of $0.000746$--$0.000856$. The joint event is estimated directly,
without assuming independence of the two divisibilities.

\texttt{S2} counts quantities divisible by $37$ among $T$, $P$,
$N$, and $\Delta$ for nine groups restricted to the
service-excluded pool: the full pool, the six chemical-axis
half-pools, and the two octets. In the standard code, $13$ of
$36$ quantities are divisible, whereas the mean in random codes
is $1.0299$.
A result of at least $13$ occurred in $39$ of $10^{6}$ trials:
$p_{\mathrm{MC}} = 4.0 \times 10^{-5}$,
$95\%$ interval $2.9 \times 10^{-5}$--$5.3 \times 10^{-5}$.
The quantities are linked by the identities $T=P+N$ and
$\Delta=P-N$ and by pairwise equality of the sums of the twin
groups Keto/Strong and Amino/Weak; sums of complementary groups
add up to the total for the full pool. These dependencies are
preserved in every permutation and are already accounted for
in the comparison with random codes.

Given these identities, the observed $13$ divisibilities reduce
to divisibility by $37$ of five quantities:
$N(\mathcal{C}_{\mathrm{SE}})$, $N_{\mathrm{SE}}(\mathrm{Keto})$,
$P_{\mathrm{SE}}(\mathrm{Amino})$, $\Delta(\mathrm{Pyrimidine})$,
and $T(\mathrm{Octet~I})$.
These are five linearly independent congruences modulo $37$;
statistical independence of the events is not assumed.
The \texttt{S2} test includes any set of $13$ or more divisibilities,
including sets that differ from the observed one.

\texttt{S3} applies the same count only to the $17$ groups
that originally contain no service codons, giving $68$
quantities in total. The standard code has $14$ divisible
quantities, against a mean of $2.1311$ in random codes:
$p_{\mathrm{MC}} = 0.025135$,
$95\%$ interval $0.024829$--$0.025443$.
Eleven divisibilities follow from
$T(\mathrm{Octet~I}) = 100 \cdot 37$, and the remaining three
form the pyrimidine $\Delta$-cascade:
$\Delta(\mathrm{Pyrimidine}) = 8 \cdot 37$ and
$\Delta(\{\mathrm{C}\}) = \Delta(\{\mathrm{T}\}) = 4 \cdot 37$.
This secondary summary thus records the propagation of two
structural cascades.

\texttt{S4sense} measures the overall proximity of group
quantities to multiples of $37$. For each of the $33$ groups
restricted to the service-excluded pool, the distances
of $T$, $P$, $N$, and $\Delta$ to the nearest multiple of $37$
are calculated; all $132$ distances are then averaged.
Zero values are included with a distance of zero.
The mean distance is $4.8182$ in the standard code and $9.1495$
in random codes:
$p_{\mathrm{MC}} = 0.000274$,
$95\%$ interval $0.000242$--$0.000307$.
The pool's atypicality is therefore also apparent in proximity
to multiples, including values that are not exactly divisible
by $37$.

In an additional post hoc check (\texttt{S4pos}), zero distances
were excluded from the \texttt{S4sense} distance set in the
standard code and in each random code. If all distances are zero,
the statistic is defined as zero; such trials are not discarded.
The mean of the remaining distances is $6.1154$ in the standard
code and $9.4195$ in random codes: nominal
$p_{\mathrm{MC}} = 0.001602$, $95\%$ interval
$0.001525$--$0.001681$.
Thus, the deviation from random codes persists after exact
hits on multiples of $37$ are excluded.

In the main model, excluding $\mathrm{ATG}$ removes whichever
amino acid was assigned to its block. To examine the role of
this choice, an additional conditional model fixes methionine
at $\mathrm{ATG}$ and permutes the other $19$ amino acids among
the remaining blocks. The results are
$p_{\mathrm{MC}} = 0.000026$ for \texttt{S2},
$0.000268$ for \texttt{S4sense}, and $0.001531$ for
\texttt{S4pos}. Atypicality persists when the set of amino acids
represented in the pool is held fixed.

The additional calculations use $10^{6}$ trials per model
in \texttt{mc\_sensitivity.py} (\texttt{seed=0}); full estimates
and intervals are provided in \texttt{mc\_sensitivity.csv}.
The independent implementation \texttt{verify\_mc\_sensitivity.py}
(\texttt{seed=12345}) reproduces the observed values exactly
and the means and tail probabilities within the expected
Monte Carlo error.

\texttt{S4a} calculates the same mean distance under Key~0.
Stop codons contribute zero, and $\mathrm{ATG}$ contributes
the profile of the amino acid assigned to its block in the
code being evaluated: methionine in the standard code and
the result of the permutation in a random code.
The mean distance is $5.5606$ in the standard code and $9.0157$
in random codes:
$p_{\mathrm{MC}} = 0.002807$,
$95\%$ interval $0.002704$--$0.002912$.
Proximity to multiples is thus retained when service codons
are restored under this formal assignment.

\texttt{S4b} evaluates the mean distance after deriving Key~1.
For the standard code and each random code, a separate minimal
non-negative assignment is derived from the Keto/Amino balance
and divisibility of the global proton and neutron sums by $37$
(Section~\ref{sec:results-key1-uniqueness}).
Divisibilities imposed by the procedure itself therefore also
arise in random codes. The mean distance is $4.6364$ in the
standard code and $7.7397$ in random codes:
$p_{\mathrm{MC}} = 0.021106$,
$95\%$ interval $0.020825$--$0.021389$.
This probability describes the proximity of group quantities
to multiples of $37$ after the same key-derivation procedure
has been applied to every code being compared.

Under the chosen model of random amino acid reassignment,
the standard code stands out primarily through the properties
of its service-excluded pool. More group quantities are exactly
divisible by $37$, and the mean distance to the nearest multiple
is smaller than is typical of random codes. After exact hits
are excluded, the remaining quantities are also closer to multiples
on average. Thus, the observed proximity is not exhausted by exact
divisibilities alone. The divisibility count and the overall mean
distance are linked: exact divisibilities enter this mean with
zero distance. These results describe different aspects of the
same arithmetic structure. Fixing methionine at $\mathrm{ATG}$
also preserves this atypicality. Proximity to multiples persists when
service codons are restored under the two formal assignments;
after Key~1 is re-derived, the deviation from random codes
is less pronounced.

All reported $p_{\mathrm{MC}}$ values are nominal probabilities
for individual summaries: no multiplicity correction was applied
to either the main summaries or the additional checks, and dependent $p$-values
are not combined by multiplication. The modulus $37$ is taken
from prior literature; the main summaries were formally defined
before the final Monte Carlo run but after inspection of the
observed structure. The analysis was not preregistered and is
exploratory. The resulting probabilities refer to the models considered here,
with the standard code's blocks fixed and the amino acids'
real nucleon profiles. They do not estimate the probability
of a natural code arising or identify a biological mechanism.
...
Section~\ref{sec:discussion-montecarlo} under the main null model,
which fixes the code's degeneracy architecture and the amino-acid
nucleon counts and randomly permutes amino acids among codon
families. An additional conditional model fixes methionine at
$\mathrm{ATG}$, preserving the set of amino acids represented in
the service-excluded pool. The observed divisibility count
(\texttt{S2}) and mean distance to the nearest multiple of $37$
(\texttt{S4sense}) are atypical under both models. These comparisons
are exploratory and depend on the choice of groups and statistics,
including the repeated counting of related quantities. They do
not estimate the probability of finding an arbitrary arithmetic
regularity after searching the data. The narrower \texttt{S3} test,
restricted to nested groups that contain no service codon at all,
is only weakly atypical ($p_{\mathrm{MC}} \approx 0.025$) and mainly
reflects copies of the divisibility of the Octet~I total
$T(\mathrm{Octet~I})$. The choice of null is not unique: other
generative models, for example randomizing the block sizes
themselves or the nucleon counts, would probe different aspects
of the regularity and are left for future work, as is any account
of a mechanism that could produce it.
...
over the sixty codons of $\mathcal{C}_{\mathrm{SE}}$, the neutron
pool is an exact multiple of the modulus,
$N(\mathcal{C}_{\mathrm{SE}}) = 3589 =
...
with $N_{\mathrm{SE}}(\mathrm{Octet~II})$ as
$1700 + 1889 = 3589$; and
...
complete table: the same divisibility in the service-excluded
pool, the same localized pair, and the same proton parity that
hold with
...
the proton sum of the service-excluded pool from the odd
...
summaries of the service-excluded pool place the standard code
in the far tail: the discrete \texttt{S2} count gives
$p_{\mathrm{MC}} = 4.0 \times 10^{-5}$, and the \texttt{S4sense} mean
distance gives $p_{\mathrm{MC}} = 2.74 \times 10^{-4}$. The narrower
\texttt{S3} test, restricted to groups that contain no service
codon at all, is only weakly atypical
($p_{\mathrm{MC}} \approx 0.025$) and mainly
...
multiplication.
Additional post hoc checks show that the deviation persists after
zero distances are excluded and when methionine is fixed at
$\mathrm{ATG}$. These results characterize atypicality under the
combinatorial models considered here, distinct from specificity
among the $27$ natural codes, and do not indicate a biological
mechanism. Other null models are left for future work.
